...
/* This is the SWI-Prolog version
   of Autotest. See doc/autotest.html
   for instructions.
*/
...
*/
'$call_giving_sfe'( Goal, Status ) :-
    catch( Goal, E, true ),
    ( var(E) ->
        Status = succeeds
    ;   
        Status = crashes_with_error_code(E)
    ), !.

'$call_giving_sfe'( _, fails ) :- !.
...
'$matches_sfe'( must_pass(_), FE, Text ) :-
...
'$matches_sfe'( Expected, Actual, 'should have'...Expectedly...'but'...Actually<>nl ) :-
